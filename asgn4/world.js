...
var _tmp = [0, 0, 0];
var _normalTmp = [0, 1, 0];

function pushVertex(out, x, y, z, r, g, b, u, v, texSlot, nx, ny, nz) {
  out.push(x, y, z, r, g, b, u, v, texSlot, nx, ny, nz);
}

function normalizeInto(out, x, y, z) {
...
function computeFaceNormal(ax, ay, az, bx, by, bz, cx, cy, cz, out) {
  var ux = bx - ax;
  var uy = by - ay;
  var uz = bz - az;
  var vx = cx - ax;
  var vy = cy - ay;
  var vz = cz - az;
  normalizeInto(out, uy * vz - uz * vy, uz * vx - ux * vz, ux * vy - uy * vx);
}
...
function appendCube(out, model, texSlot, colorOverride) {
  for (let i = 0; i < UNIT_CUBE_TRIS.length; i++) {
    ...
    pushVertex(out, _tmp[0], _tmp[1], _tmp[2], c[0], c[1], c[2], v.u, v.v, texSlot, v.n[0], v.n[1], v.n[2]);
  }
}
...
function appendUnitCubeFullTextureFaces(out, model, texSlot, cr, cg, cb) {
  ...
  function q(a, b, c, d) {
    ...
    computeFaceNormal(ax, ay, az, bx, by, bz, cx, cy, cz, _normalTmp);
    out.push(ax, ay, az, cr, cg, cb, 0, 0, texSlot, _normalTmp[0], _normalTmp[1], _normalTmp[2]);
    out.push(bx, by, bz, cr, cg, cb, 1, 0, texSlot, _normalTmp[0], _normalTmp[1], _normalTmp[2]);
    out.push(cx, cy, cz, cr, cg, cb, 1, 1, texSlot, _normalTmp[0], _normalTmp[1], _normalTmp[2]);
    out.push(ax, ay, az, cr, cg, cb, 0, 0, texSlot, _normalTmp[0], _normalTmp[1], _normalTmp[2]);
    out.push(cx, cy, cz, cr, cg, cb, 1, 1, texSlot, _normalTmp[0], _normalTmp[1], _normalTmp[2]);
    out.push(dx, dy, dz, cr, cg, cb, 0, 1, texSlot, _normalTmp[0], _normalTmp[1], _normalTmp[2]);
  }
  q(5, 1, 2, 6);
  q(0, 4, 7, 3);
  q(6, 7, 4, 5);
...
function appendUnitCubeAtlasRects(out, model, texSlot, cr, cg, cb, faceRects) {
  ...
  function qFace(a, b, c, d, ri) {
    ...
    var dz = _boxCornerBuf[d * 3 + 2];
    computeFaceNormal(ax, ay, az, bx, by, bz, cx, cy, cz, _normalTmp);
    out.push(ax, ay, az, cr, cg, cb, u0, v0, texSlot, _normalTmp[0], _normalTmp[1], _normalTmp[2]);
    out.push(bx, by, bz, cr, cg, cb, u1, v0, texSlot, _normalTmp[0], _normalTmp[1], _normalTmp[2]);
    out.push(cx, cy, cz, cr, cg, cb, u1, v1, texSlot, _normalTmp[0], _normalTmp[1], _normalTmp[2]);
    out.push(ax, ay, az, cr, cg, cb, u0, v0, texSlot, _normalTmp[0], _normalTmp[1], _normalTmp[2]);
    out.push(cx, cy, cz, cr, cg, cb, u1, v1, texSlot, _normalTmp[0], _normalTmp[1], _normalTmp[2]);
    out.push(dx, dy, dz, cr, cg, cb, u0, v1, texSlot, _normalTmp[0], _normalTmp[1], _normalTmp[2]);
  }
  qFace(5, 1, 2, 6, 0);
  qFace(0, 4, 7, 3, 1);
  qFace(6, 7, 4, 5, 2);
...
function pushWallQuadWorld(arr, ax, ay, az, bx, by, bz, cx, cy, cz, dx, dy, dz, texSlot) {
  computeFaceNormal(ax, ay, az, bx, by, bz, cx, cy, cz, _normalTmp);
  arr.push(ax, ay, az, 1, 1, 1, 0, 0, texSlot, _normalTmp[0], _normalTmp[1], _normalTmp[2]);
  arr.push(bx, by, bz, 1, 1, 1, 1, 0, texSlot, _normalTmp[0], _normalTmp[1], _normalTmp[2]);
  arr.push(cx, cy, cz, 1, 1, 1, 1, 1, texSlot, _normalTmp[0], _normalTmp[1], _normalTmp[2]);
  arr.push(ax, ay, az, 1, 1, 1, 0, 0, texSlot, _normalTmp[0], _normalTmp[1], _normalTmp[2]);
  arr.push(cx, cy, cz, 1, 1, 1, 1, 1, texSlot, _normalTmp[0], _normalTmp[1], _normalTmp[2]);
  arr.push(dx, dy, dz, 1, 1, 1, 0, 1, texSlot, _normalTmp[0], _normalTmp[1], _normalTmp[2]);
}
    ...
    for (let ix = 0; ix < MAP_SIZE; ix++) {
      ...
      for (lay = 0; lay < h; lay++) {
        ...
        if (!blockAtLayer(ix + 1, iz, lay)) {
          pushWallQuadWorld(arr, x1, y0, z1, x1, y0, z0, x1, y1, z0, x1, y1, z1, slot);
        }
        if (!blockAtLayer(ix - 1, iz, lay)) {
          pushWallQuadWorld(arr, x0, y0, z0, x0, y0, z1, x0, y1, z1, x0, y1, z0, slot);
        }
      ...
      const yTop = baseY + h;
      pushWallQuadWorld(arr, x0, yTop, z1, x1, yTop, z1, x1, yTop, z0, x0, yTop, z0, slot);
    }
  }
}
...
function pushTerrainVert(out, px, py, pz, u, v, nx, ny, nz) {
  out.push(px, py, pz, 1, 1, 1, u, v, 4, nx, ny, nz);
}
...
function appendTerrainCell(out, x, z, step) {
  ...
  computeFaceNormal(x, y00, z, x1, y10, z, x, y01, z1, _normalTmp);
  _normalTmp[0] *= -1;
  _normalTmp[1] *= -1;
  _normalTmp[2] *= -1;
  pushTerrainVert(out, x, y00, z, 0, 0, _normalTmp[0], _normalTmp[1], _normalTmp[2]);
  pushTerrainVert(out, x1, y10, z, 1, 0, _normalTmp[0], _normalTmp[1], _normalTmp[2]);
  pushTerrainVert(out, x, y01, z1, 0, 1, _normalTmp[0], _normalTmp[1], _normalTmp[2]);
  computeFaceNormal(x1, y10, z, x1, y11, z1, x, y01, z1, _normalTmp);
  _normalTmp[0] *= -1;
  _normalTmp[1] *= -1;
  _normalTmp[2] *= -1;
  pushTerrainVert(out, x1, y10, z, 1, 0, _normalTmp[0], _normalTmp[1], _normalTmp[2]);
  pushTerrainVert(out, x1, y11, z1, 1, 1, _normalTmp[0], _normalTmp[1], _normalTmp[2]);
  pushTerrainVert(out, x, y01, z1, 0, 1, _normalTmp[0], _normalTmp[1], _normalTmp[2]);
}
...
function buildWorldGeometry() {
  ...
  const cy = 8;
  const cz = MAP_SIZE / 2;
  // Negative scale flips normals inward so the sky reads correctly from inside (lecture trick).
  const skyScale = -380;
